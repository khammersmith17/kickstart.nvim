...
-- setting cursor option to be block
...
vim.api.nvim_create_autocmd('BufWritePost', {
  ...
  callback = function(args)
    -- Get the full path of the file just written
...
-- SAFE auto-open Telescope only on blank start, after plugins/UI are ready
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if client and client.server_capabilities.documentHighlightProvider then
            local buf = event.buf
            local gid = vim.api.nvim_create_augroup('kickstart-lsp-highlight-' .. buf, { clear = true })
            vim.b[buf].lsp_highlight_group = gid
      ...
      vim.api.nvim_create_autocmd('LspDetach', {
        ...
        callback = function(event)
          ...
          local gid = vim.b[event.buf].lsp_highlight_group
          if gid then
            pcall(vim.api.nvim_clear_autocmds, { group = gid, buffer = event.buf })
...
-- key map for telescope
vim.keymap.set('n', '<S-T>', ':Telescope<CR>')
